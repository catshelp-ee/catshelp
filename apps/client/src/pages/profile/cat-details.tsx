...
const personalityOptions = {
  ...
  dislikesTouching: {
    et: 'Ei lase puudutada ennast',
    en: "Doesn't allow touching",
    ru: 'Не позволяет прикасаться',
  },
...
const likesOptions = {
  ...
  likesAttention: {
    et: 'Palju tähelepanu',
    en: 'Lots of attention',
    ru: 'Много внимания',
  },
  likesPlayingWithPeople: {
    et: 'Mängida mänguasjadega inimesega',
    en: 'Play with toys with human',
    ru: 'Играть с игрушками с человеком',
  },
  likesPlayingAlone: {
    et: 'Mängida mänguasjadega üksinda',
    en: 'Play with toys alone',
    ru: 'Играть с игрушками одна',
  },
};

const habitsOptions = {
  usesLitterbox: {
    et: 'Kasutab liivakasti hästi',
    en: 'Uses litter box well',
    ru: 'Хорошо использует лоток',
  },
  usesScratchingpost: {
    et: 'Kasutab kratsimispuud hästi',
    en: 'Uses scratching post well',
    ru: 'Хорошо использует когтеточку',
  },
  selectiveWithFood: {
    et: 'On valikuline toiduga, pirtsutab',
    en: 'Picky with food, splashes',
    ru: 'Привередлив с едой, разбрасывает',
  },
  adaptable: {
    et: 'Harjub kiirelt uue kohaga',
    en: 'Adapts quickly to new place',
    ru: 'Быстро привыкает к новому месту',
  },
  scratchesFurniture: {
    et: 'Kipub kratsima muud mööblit',
    en: 'Tends to scratch other furniture',
    ru: 'Склонен царапать другую мебель',
  },
  trusting: {
    et: 'Usaldab inimesi kiiresti',
    en: 'Trusts people quickly',
    ru: 'Быстро доверяет людям',
  },
};

const CatDetailsHeader: React.FC<{ selectedCat: Profile }> = ({
  selectedCat,
}) => (
...
const CatMainInfo: React.FC<{
  ...
  const setValue = (key: string, value: string | boolean) => {
    const updatedCatInfo = {
      ...selectedCat,
      mainInfo: { ...selectedCat.mainInfo, [key]: value },
    };
  ...
  return (
    <div className="profile-section">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 text-sm">
        <div>
          ...
          <p className="section-value">
            {formatDate(selectedCat.mainInfo.birthDate, 'YYYY-MM-DD')}
          </p>
        </div>
        <div>
          <p className="section-label">Päästmiskuupäev</p>
          <p className="section-value">
            {formatDate(selectedCat.mainInfo.rescueDate, 'YYYY-MM-DD')}
          </p>
        </div>
        <div>
      ...
      <div className="mt-6 space-y-4">
        <div>
          <Label htmlFor="timeInCare">Aeg hoolduses</Label>
          <Input
            id="timeInCare"
            value="TODO"
            onChange={(e) => {}}
        ...
        <div>
          <Label htmlFor="chronicIllness">Krooniline haigus</Label>
          <Input
            id="chronicIllness"
            value={selectedCat.mainInfo.chronicConditions || ''}
            onChange={(e) => {
              setValue('chronicConditions', e.target.value);
            }}
        ...
        <div>
          <Label htmlFor="rescueStory">Päästmise lugu</Label>
          <Textarea
            id="rescueStory"
            value={selectedCat.mainInfo.rescueStory || ''}
            onChange={(e) => {
              setValue('rescueStory', e.target.value);
            }}
            rows={3}
...
const CatPersonalityInfo: React.FC<{
  ...
  const setValue = (key: string, value: string | boolean) => {
    const updatedCatInfo = {
      ...selectedCat,
      personalityInfo: { ...selectedCat.personalityInfo, [key]: value },
    };
    setSelectedCat(updatedCatInfo);
  };
  ...
  return (
    <div className="profile-section">
      ...
      <div className="space-y-6">
        <div>
          <Label className="block">Iseloomujooned</Label>
        ...
        <div>
          <Label className="block">Kassile meeldib</Label>
        ...
        <div>
          <Label className="block">Harjumused ja käitumine</Label>
        ...
        <div>
          ...
          <Textarea
            ...
            onChange={(e) => {
              setMainInfoValue('description', e.target.value);
            }}
            rows={3}
        ...
        <div>
          <Label htmlFor="specialRequirements">Erisoovid uuele perele</Label>
          ...
          <Textarea
            ...
            rows={3}
            placeholder={
              'Nt vajab kõrvale kassi, ilma lasteta pere, rahulikuma eluviisiga pere, kass vajab suurt tähelepanu jne'
            }
            className="resize-y"
          />
        </div>
  ...
  return (
    <>
      ...
      {!isMobile && (
        <ImageGallery
          animalId={selectedCat.animalId}
          name={selectedCat.mainInfo.name}
          images={[]}
        />
      )}
    </>
